...
/****** Object:  View [dbo].[SSRS_vw_Inbound_Receiving_Detail]    Script Date: 8/5/2021 5:50:01 PM ******/
SET ANSI_NULLS ON
GO
...
CREATE VIEW [dbo].[SSRS_vw_Outbound_Picked_Detail]
AS
SELECT        RTRIM(Number) AS Number, RTRIM(TradingPartnerCode) AS TradingPartnerCode, RTRIM(TradingPartnerDescription) AS TradingPartnerDescription, RTRIM(LineNumber) AS LineNumber, RTRIM(Code) AS Code, 
                         ...
                         DocumentUOMConversion, RTRIM(ItemUOM) AS ItemUOM, TrackingEntity, Batch, CONVERT(Date,ExpiryDate,101) as ExpiryDate, TXActionQty
FROM            (SELECT        dbo.[Document].Number, dbo.[Document].TradingPartnerCode, dbo.[Document].TradingPartnerDescription, dbo.DocumentDetail.LineNumber, dbo.MasterItem.Code, dbo.MasterItem.Description, 
													...
													TSQL_1.Barcode as TrackingEntity, TSQL_1.Batch as Batch, TSQL_1.ExpiryDate as ExpiryDate,
                                                    ...
                                                    dbo.DocumentDetail.UOM AS DocumentUOM, dbo.DocumentDetail.UOMConversion AS DocumentUOMConversion, TXActionQty
                          FROM            dbo.MasterItem RIGHT OUTER JOIN
                                                    ...
                                                    dbo.Users RIGHT OUTER JOIN
                                                        (SELECT        Transaction_1.DocumentLine_id, Transaction_1.User_id AS UserID, Transaction_1.Date AS TransactionDate, TrackingEntity.Barcode, TrackingEntity.Batch, TrackingEntity.ExpiryDate, SUM(Transaction_1.ActionQty) as TXActionQty
                                                          ...
                                                          GROUP BY dbo.TrackingEntity.MasterItem_id, Transaction_1.DocumentLine_id, Transaction_1.User_id, Transaction_1.Date, TrackingEntity.Barcode, TrackingEntity.Batch, TrackingEntity.ExpiryDate) AS TSQL_1 ON dbo.Users.ID = TSQL_1.UserID ON 
                          ...
                          WHERE        (dbo.Type.Name IN ('ORDER'))) AS TSQL
GROUP BY Number, TradingPartnerCode, LineNumber, Code, Description, ItemCategory, ItemType, TransactionType, [User], TradingPartnerDescription, DocumentCreateDate, DocumentActionDate, DocumentStatus, DetailStatus, 
                         DocumentUOM, DocumentUOMConversion, ItemUOM, TrackingEntity,Batch,ExpiryDate, TXActionQty
GO
